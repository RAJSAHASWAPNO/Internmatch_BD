...
class ApplyJobView(StudentRequiredMixin, View):
    """
    Student applies to a job. Kept as View (not CreateView)
...
class DeleteBookmarkView(StudentRequiredMixin, DeleteView):
    """Student deletes a saved bookmark."""
    model = BookmarkJob
    pk_url_kwarg = 'id'
...
class JobBookmarkView(StudentRequiredMixin, View):
    """
    Student bookmarks a job. Kept as View (not CreateView)
    because it has custom duplicate-prevention logic.
    """
    def post(self, request, id):
        user = request.user
