...
from jobapp.models import Job, Skill
...
class SEOTestCase(TestCase):
    def setUp(self):
        ...
        self.skill = Skill.objects.create(name='IT')
    ...
    def test_homepage_seo(self):
        ...
        self.assertContains(response, '<title>INTERMATCHBD - Find Your Dream Job Today</title>')
        self.assertContains(response, 'meta name="description" content="INTERMATCHBD is the leading job board')

    def test_job_detail_seo(self):
        response = self.client.get(reverse('jobapp:single-job', kwargs={'id': self.job.id}))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, '<title>Software Engineer at Test Company | INTERMATCHBD</title>')
